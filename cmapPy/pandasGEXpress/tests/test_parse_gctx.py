...
class TestParseGctx(unittest.TestCase):
    ...
    def test_check_and_order_id_inputs(self):
        ...
        # case 5: sort_row_meta and sort_col_meta differ - an out-of-range ridx must still be
        # validated (and raise) when sort_row_meta=True, regardless of sort_col_meta. This is a
        # regression test for a bug where the row branch used sort_col_meta instead of
        # sort_row_meta internally, silently skipping row idx validation in this scenario.
        out_of_range_ridx = [0, 10]
        with self.assertRaises(Exception):
            parse_gctx.check_and_order_id_inputs(None, out_of_range_ridx, None, None, row_meta, col_meta,
                                                  sort_row_meta = True, sort_col_meta = False)
