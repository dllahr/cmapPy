...
class TestDifferential(unittest.TestCase):
    ...
    def test_diff_gctoo_vc_median_norm(self):
        # median_norm with plate_control=False should normalize against the medians of
        # just the negative control samples (E, F here), not all samples' medians
        vc_median_normed = diff_gctoo.diff_gctoo(test_gctoo, plate_control=False, diff_method="median_norm")
        pc_median_normed = diff_gctoo.diff_gctoo(test_gctoo, plate_control=True, diff_method="median_norm")
        self.assertFalse(vc_median_normed.data_df.equals(pc_median_normed.data_df))

        # neg control (E, F) medians per row are [7.5, 7.0, 6.0]
        self.assertEqual(vc_median_normed.data_df.loc[0, "A"], 4 - 7.5)
        self.assertEqual(vc_median_normed.data_df.loc[1, "E"], 8 - 7.0)
        self.assertEqual(vc_median_normed.data_df.loc[2, "F"], 6 - 6.0)
